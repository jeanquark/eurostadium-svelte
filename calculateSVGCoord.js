...
const calculateSVGCoord = async () => {
    ...
    // for (const country in coordData) {
        ...
        for (let i = 0; i < array.length; i++) {
            array[i]['venue']['x'] = parseInt(x_translate) + (parseFloat(array[i]['venue']['x']) - leftLng)*(x_max/Math.abs(rightLng-leftLng));
            array[i]['venue']['y'] = parseInt(y_translate) + (parseFloat(array[i]['venue']['y']) - topLat)*(y_max/Math.abs(bottomLat-topLat));
        }
